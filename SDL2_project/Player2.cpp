...
#pragma region Keyboard Input
	...
	// Fire Rocket
	if (mInputManager->KeyDown(SDL_SCANCODE_LCTRL) && !shield)
	{
		FireRocket();
	}

	// Raise Shield 
	if (mInputManager->KeyDown(SDL_SCANCODE_LSHIFT) && mScore > 0)
	{
		mShieldFront->Update();
		mShieldBack->Update();
		mShieldUp->Update();
	...
	// Lower Shield
	if (mInputManager->KeyReleased(SDL_SCANCODE_LSHIFT))
	{
		shield = false;
	}
	//}
